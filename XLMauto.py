...
# 시작 메세지 슬랙 전송
post_message(myToken,"#rbhrec", "XLM 자동매매 시작!")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            target_price = get_target_price("KRW-XLM", 0.4)
            ...
            if target_price < current_price and ma15 < current_price:
                ...
                if krw > 5000:
                    ...
                    post_message(myToken,"#rbhrec", "XLM buy : " +str(buy_result))
        else:
            xlm = get_balance("XLM")
                if xlm > 7:
                sell_result = upbit.sell_market_order("KRW-XLM", xlm*0.9995)
                post_message(myToken,"#rbhrec", "XLM sell : " +str(sell_result))
        time.sleep(1)
    except Exception as e:
        print(e)
        post_message(myToken,"#rbhrec", "XLM Error : " + e)
        time.sleep(1)
