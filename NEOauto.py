...
# 시작 메세지 슬랙 전송
post_message(myToken,"#rbhrec", "NEO 자동매매 시작!")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            target_price = get_target_price("KRW-NEO", 0.4)
            ma15 = get_ma15("KRW-NEO")
            current_price = get_current_price("KRW-NEO")
            if target_price < current_price and ma15 < current_price:
                krw = get_balance("KRW")
                if krw > 5000:
                    buy_result = upbit.buy_market_order("KRW-NEO", krw*0.9995)
